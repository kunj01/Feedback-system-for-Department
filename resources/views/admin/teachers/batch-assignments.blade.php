    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-xl shadow-sm p-5 mb-5">
            <div class="flex items-center justify-between">
                ...
                <button onclick="openAssignBatchModal()" class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold rounded-lg flex items-center">
...
<!-- Assign Batch Modal -->
<div id="assignBatchModal" class="fixed inset-0 bg-gray-900 bg-opacity-75 backdrop-blur-sm hidden flex-col items-center justify-center z-50" style="display: none;">
    <div class="bg-white rounded-xl shadow-2xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-hidden">
        <div class="p-5 border-b border-gray-200 flex items-center justify-between">
            ...
            <button onclick="closeAssignBatchModal()" type="button" class="text-gray-400 hover:text-gray-600">
        ...
        <form id="assignBatchForm" class="overflow-y-auto" style="max-height: calc(90vh - 140px);">
            ...
            <div class="p-5 space-y-4">
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <h4 class="text-sm font-semibold text-gray-700 mb-3">Filter Batches & Subjects</h4>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs font-medium text-gray-700 mb-1">Semester</label>
                            <select id="filterSemester" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" onchange="updateDivisionFilter(); filterAllSelects();">
                        ...
                        <div>
                            ...
                            <select id="filterDivision" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" onchange="filterAllSelects()">
...
console.log('=== Batch Assignment Module Loaded ===');
...
function addAssignmentRow() {
    ...
    row.className = 'assignment-row border border-gray-200 rounded-lg p-4 bg-gray-50';
    ...
    row.innerHTML = `
        ...
        <div class="grid grid-cols-2 gap-3">
            <div>
                ...
                <select name="batch_ids[]" required class="batch-select w-full px-3 py-2 text-sm border border-gray-300 rounded-lg">
                    <option value="">Select Batch</option>
                </select>
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-700 mb-1">Subject</label>
                <select name="subject_ids[]" class="subject-select w-full px-3 py-2 text-sm border border-gray-300 rounded-lg">
                    <option value="">Select Subject</option>
                </select>
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-700 mb-1">Type *</label>
                <select name="types[]" required class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg">
            ...
            <div>
                ...
                <input type="text" name="notes[]" placeholder="Optional notes" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg">
            </div>
        </div>
    `;
    ...
    divisions.forEach(division => {
        if (division.batches && division.batches.length > 0) {
            division.batches.forEach(batch => {
                ...
                option.dataset.semester = division.semester;
                option.dataset.division = division.id;
                option.textContent = `${division.name} - ${batch.batch_name}`;
    ...
    subjects.forEach(subject => {
        ...
        option.textContent = `${subject.name} (Sem ${subject.semester})`;
...
// Form submission
document.getElementById('assignBatchForm').addEventListener('submit', async function(e) {
    e.preventDefault();
